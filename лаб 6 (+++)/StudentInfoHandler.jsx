...
export class StudentInfoHandler extends React.Component {
    ...
    age(student) {
        ...
        birthMonth = Math.round((birthDate / 10000) % 10),
        birthDay = Math.round(birthDate / 1000000),
        age = currentDate.getFullYear() - birthYear;
        if (
            currentDate.getMonth() < birthMonth - 1 // getMonth - месяцы начинаются с нулевого
        ) 
            age--;
        if (
    ...
    mailOperator(student) {
        return student.email.split("@")[1]; // split - разделение кода, где в круглых скобках символ разделитель, а в квадратных навправление (0 - перед символом, 1 - после символа)
    }
    ...
    phoneOperator(student) {
        ...
        code = +phone.slice(position, position + 2); // + - численное преобразование
